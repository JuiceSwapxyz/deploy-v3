...
async function createPoolAndAddLiquidity() {
  ...
  const CUSTOM_TOKEN_ADDRESS = '0x36c16eaC6B0Ba6c50f494914ff015fCa95B7835F'
  ...
  const CUSTOM_TOKEN_AMOUNT = ethers.utils.parseUnits('2', 9) // 2 Custom Token (9 decimals)

  // Connect to Citrea
  const provider = new ethers.providers.JsonRpcProvider(CITREA_RPC, 5115)
  const wallet = new ethers.Wallet(PRIVATE_KEY, provider)

  console.log('🏊 Creating JuiceSwap V3 Pool: WCBTC/Custom Token')
  console.log('=========================================')
  ...
  console.log('  Custom Token:', ethers.utils.formatUnits(CUSTOM_TOKEN_AMOUNT, 9))
  console.log('')
  ...
  const customToken = new ethers.Contract(CUSTOM_TOKEN_ADDRESS, ERC20_ABI, wallet)

  try {
    // Check balances
    const wcbtcBalance = await wcbtc.balanceOf(wallet.address)
    const customTokenBalance = await customToken.balanceOf(wallet.address)

    console.log('Current Balances:')
    console.log('  WCBTC:', ethers.utils.formatEther(wcbtcBalance))
    console.log('  Custom Token:', ethers.utils.formatUnits(customTokenBalance, 9))
    console.log('')

    if (wcbtcBalance.lt(WCBTC_AMOUNT)) {
      throw new Error('Insufficient WCBTC balance')
    }
    if (customTokenBalance.lt(CUSTOM_TOKEN_AMOUNT)) {
      throw new Error('Insufficient Custom Token balance')
    }

    // Order tokens (token0 < token1)
    const [token0, token1, amount0, amount1] = WCBTC_ADDRESS.toLowerCase() < CUSTOM_TOKEN_ADDRESS.toLowerCase()
      ? [WCBTC_ADDRESS, CUSTOM_TOKEN_ADDRESS, WCBTC_AMOUNT, CUSTOM_TOKEN_AMOUNT]
      : [CUSTOM_TOKEN_ADDRESS, WCBTC_ADDRESS, CUSTOM_TOKEN_AMOUNT, WCBTC_AMOUNT]

    console.log('Token Order:')
    console.log('  Token0:', token0 === WCBTC_ADDRESS ? 'WCBTC' : 'Custom Token')
    console.log('  Token1:', token1 === WCBTC_ADDRESS ? 'WCBTC' : 'Custom Token')
    console.log('')
    ...
    console.log('🔓 Approving Custom Token...')
    const customTokenApproveTx = await customToken.approve(POSITION_MANAGER_ADDRESS, CUSTOM_TOKEN_AMOUNT, {
      gasLimit: 100000
    })
